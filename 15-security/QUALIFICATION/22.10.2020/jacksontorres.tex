...
\item
  O segundo capítulo buscará realizar o levantamento teórico a respeito dos
  conceitos, aspectos fundamentais e dados amplos para compreensão da
  evolução do setor e \emph{spread} bancário no Brasil, por meio de pesquisa
  bibliográfica e busca em banco de dados.
\item
  O terceiro capítulo tem como anseio identificar e testar a variáveis macroeconômicas e microeconômicas que influenciam os componentes do \emph{spread} bancário \emph{ex-post} através da coleta, análises, tratamento de dados e construção de modelo econométrico utilizando dados em painel.
\item
  O terceiro capítulo, ainda visa verificar como as variações dos componentes do \emph{spread} bancário afetam a rentabilidade dos bancos por meio da coleta, análises e tratamento de dados e definição de modelo econométrico com utilização de dados em painel.
\item
  O quarto capítulo almeja explanar e analisar, diante as premissas empregadas, os resultados obtidos na pesquisa, visando descrever e embasar as relações entre as variáveis.
\item
