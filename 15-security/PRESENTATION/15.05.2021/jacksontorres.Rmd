...
\vspace{-4mm}
```{r concetration, fig.width=6, fig.height=4}
con.banks <- 
        em.banks %>%
        transform(Ano = as.integer(Ano)) %>% 
        filter(Mês == 'Dez') %>%
...
\vspace{-4mm}
```{r capital.graphic, fig.width=6, fig.height=4}
ev.o.banks <- 
        o.banks %>% 
...
\vspace{-4mm}
```{r credit.gdp, fig.width=6, fig.height=4}
...
\vspace{-4mm}
```{r balance.credit, fig.width=6, fig.height=4}
...
\vspace{-4mm}
```{r money.base.d, fig.width=6, fig.height=4}
...
\vspace{-4mm}
```{r m2m3m4, fig.width=6, fig.height=4}
...
saveRDS(fit.pvargmm,  file = "~/Desktop/04-DISSERTATION/01-data/model/fit.pvargmm.f.RDS")

saveRDS(sumPvarGmm,  file = "~/Desktop/04-DISSERTATION/01-data/model/sumPvarGmm.f.RDS")
```
...
hansen_j_test(fit.pvargmm) %>% as.data.frame() %>% 
  rename(Estatística = statistic, 
         ...
         Número.Instrumentos = nof_instruments,
         Método = method) %>%  
  kbl(booktabs = TRUE) %>%
