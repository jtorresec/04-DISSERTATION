...
hist(banksPanel.00$SprEp, main = "Tratamento",
     xlab = "Spread Ex-post")
...
testgvmla <- gvlma(SprEp ~ DAdm + DesCap + GrCon + OtDes
          + Inad #+ RcPd 
          + Int + EPr
          + lnComp + ImpInd + ImpRend
          + DepAv + DepAp + DepPop 
...
model.pvrgmm <- 
        ...
        select("DATA", "SprEp", "TxApl", "TxCap", "BANCO", "Rent", "DAdm",
               ...
               "GrCon", "Id_Banco", "Id_Data", "Id_TpIns"
          ) %>% 
        as.data.frame()
...
library(caret)
set.seed(1234)
pvar.train <- createDataPartition(model.pvrgmm$SprEp, p = 0.40, list = FALSE)
